...
public class Registry implements HubRegistryInterface {
	...
	private volatile GridMonitor gridMonitorMBean;
	...
	public static Registry newInstance(HubInterface hub,
	 ...
	 * 
	 * @see
	 * org.openqa.grid.internal.HubRegistryInterface#setNewSessionWaitTimeout
	 * (int)
	 */
	@Override
	...
	public void removeIfPresent(RemoteProxy proxy) {
		...
		// an empty TestSlot list, which doesn't figure into the proxy
		// equivalence
		// check. Since we want to free up
		// those test sessions, we need to operate on that original object.
		if (proxies.contains(proxy)) {
			log.warning(String
					.format("Proxy '%s' was previously registered.  Cleaning up any stale test sessions.",
							proxy));
//			getGridMonitorMBean().setLastRegistrationString(proxy + " PREVREG");

			final RemoteProxy p = proxies.remove(proxy);
			for (TestSlot slot : p.getTestSlots()) {
				forceRelease(slot,
						SessionTerminationReason.PROXY_REREGISTRATION);
			}
			p.teardown();
		}
	}
	 ...
	 * 
	 * @see
	 * org.openqa.grid.internal.HubRegistryInterface#forceRelease(org.openqa
	 * .grid .internal.TestSlot,
	...
	public void addNewSessionRequest(RequestHandler handler) {
		try {
			...
			proxies.verifyAbilityToHandleDesiredCapabilities(handler
					.getRequest().getDesiredCapabilities());
	...
	private void assignRequestToProxy() {
		while (!stop) {
			try {
				...
				// Just make sure we delete anything that is logged on this
				// thread from
				// memory
				...
				log.log(Level.SEVERE, "Unhandled exception in Matcher thread.",
						t);
			}
		}
	...
	public void add(RemoteProxy proxy) {
		...
		try {
			...
			if (registeringProxies.contains(proxy)) {
				log.warning(String
						.format("Proxy '%s' is already queued for registration.",
								proxy));
//				getGridMonitorMBean().setLastRegistrationString(
//						proxy + " ALRDYQUED");
...
//			getGridMonitorMBean().setLastRegistrationString(proxy.toString());
			fireMatcherStateChanged();
		} finally {
			lock.unlock();
		}
	 ...
	 */
	@Override
	public void setThrowOnCapabilityNotPresent(
	 ...
	 * 
	 * @see
	 * org.openqa.grid.internal.HubRegistryInterface#getExistingSession(org.
	 * openqa .grid.internal.ExternalSessionKey)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * org.openqa.grid.internal.HubRegistryInterface#getDesiredCapabilities()
	 */
	@Override
	 ...
	 * 
	 * @see
	 * org.openqa.grid.internal.HubRegistryInterface#getProxyById(java.lang.
	 * String )
	 */
	@Override
	...
	private static class UncaughtExceptionHandler implements
		...
		public void uncaughtException(Thread t, Throwable e) {
			log.log(Level.SEVERE,
					"Matcher thread dying due to unhandled exception.", e);
		}
	}
	...
	public GridMonitor getGridMonitorMBean() {
		return gridMonitorMBean;
	}

	public void setGridMonitorMBean(GridMonitor gridMonitorMBean) {
		this.gridMonitorMBean = gridMonitorMBean;
	}
